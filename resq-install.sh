...
function get_ip() {
  declare -a NODE_resq
  for resq in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_resq+=($(curl --interface $resq --connect-timeout 2 -s4 icanhazip.com))
  done
...
function important_information() {
 ...
 echo -e "Please check that your chain is fully synced before starting it from local wallet. To do this type resq-cli mnsync status. ${GREEN}RequestedMasternodeAssets${NC} must equal ${GREEN}999${NC}"
 echo -e "================================================================================================================================"
}
...
NC='\033[0m'
    ...
    echo -e "${MENU}*********************************************${NORMAL}"
    echo -e "${MENU}****Welcome to the RESQ Masternode setup******${NORMAL}"
    echo -e "${MENU}*********************************************${NORMAL}"
